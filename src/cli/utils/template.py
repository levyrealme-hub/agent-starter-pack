            ...
            if is_remote and remote_template_path:
                ...
                if remote_pyproject.exists():
                    try:
                        ...
                        has_starter_pack_integration = "[tool.agent-starter-pack]" in remote_pyproject_content
                        if not has_starter_pack_integration:
                            preserve_files.append("pyproject.toml")
                            logging.debug("Remote pyproject.toml lacks starter pack integration - will preserve base template version")
                        else:
                            logging.debug("Remote pyproject.toml has starter pack integration - using remote version only")
                    except Exception as e:
                        logging.warning(f"Could not read remote pyproject.toml: {e}. Will preserve base template version.")
                        preserve_files.append("pyproject.toml")
                else:
                    preserve_files.append("pyproject.toml")
                ...
                for preserve_file in preserve_files:
                    ...
                    if base_file.exists() and remote_file.exists():
                        ...
                        preserved_file = generated_project_dir / f"starter_pack_{base_name}{extension}"
                ...
                if generated_project_dir.exists():
                    ...
                    if final_destination.exists():
                        for item in final_destination.iterdir():
                            if item.is_file() and (item.name.lower().startswith("readme") or item.name == "pyproject.toml"):
                                existing_preserved_files.append((item.name, item.read_text()))
                    ...
                    for file_name, file_content in existing_preserved_files:
                        ...
                        preserved_file_path = final_destination / f"starter_pack_{base_name}{extension}"
